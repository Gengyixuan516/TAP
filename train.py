...
def training(dataset, opt, pipe, dataset_name, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, wandb=None, logger=None, ply_path=None):
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        voxel_visible_mask = prefilter_voxel(viewpoint_cam, gaussians, pipe,background)   # 预处理渲染过程中的体素（voxel），以确定哪些体素在当前视角（viewpoint_cam）下是可见的
        ...
        # loss = (1.0 - opt.lambda_dssim) * Ll1 + opt.lambda_dssim * ssim_loss + 0.01*scaling_reg + opt.lambda_mask * torch.mean((torch.sigmoid(gaussians.get_mask)))  # changed
        ...
        with torch.no_grad():
            ...
            if iteration < opt.update_until and iteration > opt.start_stat:
                ...
                if iteration > opt.update_from and iteration % opt.update_interval == 0:
                    ...
                    gaussians.my_adjust_anchor(iteration, coarse_voxel_size=opt.coarse_voxel_size, voxel_size=dataset.voxel_size, check_interval=opt.update_interval, success_threshold=opt.success_threshold, grad_threshold=opt.densify_grad_threshold, min_opacity=opt.min_opacity)
            else:
                # if iteration > opt.update_until and iteration % opt.mask_prune_iter == 0:
        ...
        # new code, plt
        log_gaussians.append(log_gaussian_iter)
        if iteration == opt.iterations:
            torch.save(log_gaussians, scene.model_path + "/log_gaussians.pth")
            # 绘制图像
            iterations = [data['iteration'] for data in log_gaussians]
            anchors = [data['anchors'] for data in log_gaussians]

            plt.figure(figsize=(10, 5))
            plt.plot(iterations, anchors, label='Number of Anchors')
            plt.xlabel('Iteration')
            plt.ylabel('Number of Anchors')
            plt.title('Number of Anchors per Iteration')
            plt.legend()
            plt.grid(True)
            plt.savefig(model_path + '/anchors_per_iteration.png')  # 保存图像
        #     plt.show()
        # end new
...
def prepare_output_and_logger(args):    
    if not args.model_path:
        if os.getenv('OAR_JOB_ID'):
            ...
            unique_str = os.path.split(args.source_path)[-1]
